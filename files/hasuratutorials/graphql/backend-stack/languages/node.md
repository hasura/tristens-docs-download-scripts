...
Create `server-codegen.ts` 

In `package.json` add a codgen script

Define `schema.graphql` which will create the schema for our GraphQL server

Run the code generator `npm run codegen` 

Create the GraphQL Yoga server

Add it to Express

Run the Express app and navigate to `<Express URL>/graphql` , if everything worked you should be able to query `posts` 
...
In the Hasura Console remote schema tab, add your Node server `<Express URL>/graphql` 

In the API Explorer tab, try querying the sample shows.

Image: [ Hasura Event Triggers with Node backend ](https://graphql-engine-cdn.hasura.io/learn-hasura/assets/backend-stack/node/node-remote-schema.png)
...
Image: [ Hasura Actions with Node backend ](https://graphql-engine-cdn.hasura.io/learn-hasura/assets/backend-stack/node/node-hasura-actions.png)
...
In the Hasura Console add a `user` table with a `Text` column `name` and the frequently used `UUID` column id.

In the event trigger tab, on the `user` table, check the insert and via console trigger operations.

The event trigger payload schema can be found[ in the docs ](https://hasura.io/docs/latest/graphql/core/event-triggers/payload/#json-payload). We make an interface in TypeScript to represent this

Now we make an REST controller that handles the event

Add the route to `src/main.ts` 

When you add a user in Hasura your Express server should receive the event.

Image: [ Hasura Event Triggers with Node backend ](https://graphql-engine-cdn.hasura.io/learn-hasura/assets/backend-stack/node/node-event-triggers.png)
...
Modify the `package.json` codegen script

In `src/action/action.ts` we add our query. We are going to fetch all users.

Run codegen `npm run codegen` 

In our action handler query all users

Call your action REST endpoint and you should see all users printed out!
...
Image: [ Promo ](https://hasura.io/learn/graphql/backend-stack/static/hasura-free-ff60e409244e0ea12b5a3045d1a9096b.png)
...
- CONTENTS
- [ GraphQL server with Node ](https://hasura.io/learn/graphql/backend-stack/languages/node/#graphqlserverwithnode)
- [ Node.js Project Setup for GraphQL Server ](https://hasura.io/learn/graphql/backend-stack/languages/node/#node.jsprojectsetupforgraphqlserver)
- [ Create a Node.js GraphQL server with GraphQL Yoga ](https://hasura.io/learn/graphql/backend-stack/languages/node/#createanode.jsgraphqlserverwithgraphqlyoga)
- [ Convert a Node.js REST API endpoint to GraphQL ](https://hasura.io/learn/graphql/backend-stack/languages/node/#convertanode.jsrestapiendpointtographql)
- [ Example: Querying GraphQL with Node Client graphql-request ](https://hasura.io/learn/graphql/backend-stack/languages/node/#example:queryinggraphqlwithnodeclientgraphql-request)
- [ Summary ](https://hasura.io/learn/graphql/backend-stack/languages/node/#summary)
